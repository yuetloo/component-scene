...
function zpad(v: string, length?: number): string {
    if (length == null) { length = 2; }
    while (v.length < length) { v = "0" + v; }
    return v;
}

export function toDotH(data: Uint8Array, tag: string): string {
    if (data.length % 2) { throw new Error("data must be uint16_t; event length"); }
